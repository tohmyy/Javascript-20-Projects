...
const tellMe=(joke)=>{
    joker = joke
    VoiceRSS.speech({
        key: '0fd1d629f15345c996f09b9a4684a9af',
        src: joke,
        hl: 'en-us',
        v: 'Linda',
        r: 0, 
...
const getJoke=async()=>{
    let joker = '';
    try{
        const url = await fetch ('https://v2.jokeapi.dev/joke/Any?blacklistFlags=nsfw&type=single')
        const response = await url.json()
        joker = response.joke
        console.log(response.joke)
        tellMe(response.joke)
    }
    catch(err){
        console.log('abeg me i dont understand again o', err)
    }
